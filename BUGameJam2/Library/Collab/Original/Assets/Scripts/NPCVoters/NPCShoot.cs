...
namespace Assets.Scripts.NPCVoters
{
    public class NPCShoot : MonoBehaviour
    {
        ...
        {
            GameObject milkShake = Instantiate(milkshakePrefab, (transform.position + new Vector3(transform.localScale.x, 0, 0)), Quaternion.LookRotation((target.position - transform.position).normalized));

            Rigidbody milkshakeRigidBody = milkShake.GetComponent<Rigidbody>();

            milkshakeRigidBody.AddForce(Vector3.forward * throwForce, ForceMode.Impulse);
        }
    }
}
//milkShake.transform.rotation.eulerAngles